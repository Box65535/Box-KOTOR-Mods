...
def write_header(file, hides):
	file.write('; This file was generated by hidegen.py\n')
	file.write('\n')
	file.write('[Settings]\n')
...
attack_high_pattern = [attack_medium, attack_medium, bonus(attack_high, 2), bonus(attack_high, 3)]
attack_medium_pattern = [attack_low, attack_medium,  bonus(attack_medium, 2), bonus(attack_high, 3)]
defense_high_pattern = [defense_medium, defense_medium, defense_high, defense_high]
defense_medium_pattern = [defense_low, defense_medium, defense_medium, defense_high]
save_high_pattern [bonus(save_low, 2), bonus(save_medium, 2), bonus(save_high, 4), bonus(save_high, 4)]
save_medium_pattern [bonus(save_low, 1), bonus(save_medium, 1), bonus(save_high, 3), bonus(save_high, 4)]
...
hides = []
codes = ['wk', 'rg', 'sr', 'bs']
for hc in hideclasses:
	for c in range(0, len(codes))
		for i in range(1, max_level):
			hides.append(Hide('eh_' + hc.name + codes[c], hc.attack[c](i), hc.defense[c](i),
			hc.fortitude[c](i), hc.reflex[c](i), hc.will[c](i), hc.regen[c](i), hc.deflect[c](i),
			hc.immunities, hc.feats, hc.skill_bonuses, hc.immunity_bonuses, hc.save_bonuses))
